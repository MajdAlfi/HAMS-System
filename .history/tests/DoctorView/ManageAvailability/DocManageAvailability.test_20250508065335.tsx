...
// Setup mock localStorage and fetch
beforeEach(() => {
...
describe("DocManageAvailability", () => {
  it("renders doctor availability form", async () => {
    ...
    const fromInput = (await waitFor(() =>
      screen.getByPlaceholderText("From HH")
    )) as HTMLInputElement;

    await waitFor(() => {
      expect(fromInput.value).toBe("09");
    });
    screen
      .getAllByRole("textbox")
      .forEach((el) => console.log((el as HTMLInputElement).placeholder, el));
  ...
  it("allows updating working hours and special dates", async () => {
    ...
    const mmInputs = screen.getAllByPlaceholderText("MM");
    fireEvent.change(mmInputs[0], { target: { value: "15" } });

    fireEvent.change(screen.getByPlaceholderText("HH"), {
      target: { value: "18" },
    });
    fireEvent.change(mmInputs[1], { target: { value: "45" } });
