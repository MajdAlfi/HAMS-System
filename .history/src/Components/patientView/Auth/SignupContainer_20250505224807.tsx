...
export const SignupContainer = () => {
  ...
  const handleSubmit = () => {
    ...
    formData.append("isDoc", isDoc.toString());
    formData.append("accountType", isDoc ? "Doctor" : "Patient");

    if (isDoc) {
      formData.append("Specialization", specialization);
      formData.append("Hospital", selectedHospital);
      formData.append("img", selectedImage!); // `!` since it's checked above
    }
    alert(formData.get("phoneNo"));
    // If your backend supports image upload, this would need FormData
    HandleRegSubmit(formData, `${baseURL}/register`).then(async (token) => {
      if (token != null) {
