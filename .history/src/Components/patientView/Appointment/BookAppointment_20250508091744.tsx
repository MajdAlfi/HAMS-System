import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { useQuery } from "../../../Services/Others/useQuery";
import { baseURL } from "../../../Services/Others/baseURL";
import { FetchDocList } from "../../../Services/API/fetchDocList";
import { doctorModel } from "../../../Models/doctorModel";
import { weeklyModel } from "../../../Models/weeklyModel";
import { bookApt } from "../../../Services/Appointments/bookApt";
import { FetchDocAvailability } from "../../../Services/API/FetchDocAvailability";

// Type Definitions
type APIResponse = {
  dataDoc: doctorModel[];
};

type DoctorInfo = {
  name: string;
  uid: string;
  Hospital: string;
};

type DateOffType = {
  Sunday: boolean;
  Monday: boolean;
  Tuesday: boolean;
  Wednesday: boolean;
  Thursday: boolean;
  Friday: boolean;
  Saturday: boolean;
};

const BookAppointment = () => {
  const [SelectedDoctor, SetSelectedDoctor] = useState<DoctorInfo | null>(null);
  const [SelectedDate, setSelectedDate] = useState<string>("");
  const [SelectedTime, setSelectedTime] = useState<string>("");
  const [ListDocs, setListDocs] = useState<APIResponse | null>(null);
  const [weekAva, setWeekAva] = useState<DateOffType | null>(null);
  const [avaTimes, setAvaTimes] = useState<string[]>([]);
  const [desc, setDesc] = useState<string>("");
  const navigate = useNavigate();
  const query = useQuery();
  const myparams = query.get("date");
  const myparamsDocs = query.get("docName");
  const myparamsHname = query.get("Hname");
  const myparamsdocUid = query.get("docUid");
  const myparamsTime = query.get("time");
  // Initialize date from query param
  useEffect(() => {
    if (myparams) {
      const parsedDate = new Date(parseInt(myparams.toString()));
      setSelectedDate(parsedDate.toISOString().split("T")[0]);
    }
  }, [myparams]);
  useEffect(() => {
    if (myparamsDocs && myparamsHname && myparamsdocUid) {
      SetSelectedDoctor({
        name: myparamsDocs,
        uid: myparamsdocUid,
        Hospital: myparamsHname,
      });
    }
  }, [myparamsDocs, myparamsHname, myparamsdocUid]);
  useEffect(() => {
    if (myparamsTime) {
      setSelectedTime(myparamsTime);
    }
  }, [myparams]);
  // Fetch doctor list on mount
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        const data = await FetchDocList(
          { "Content-Type": "application/json" },
          `${baseURL}/getDocList`
        );
        setListDocs(data);
      } catch (e) {
        console.error("Failed to fetch doctors list:", e);
      }
    };

    fetchDoctors();
  }, []);

  // Fetch availability when doctor is selected
  useEffect(() => {
    const fetchAvailability = async () => {
      if (!SelectedDoctor?.uid) return;

      try {
        const data = await FetchDocAvailability(
          {
            "Content-Type": "application/json",
            uid: SelectedDoctor.uid,
          },
          `${baseURL}/getDocAva`
        );

        const item = data.dataWeek[0] as weeklyModel;

        const availability: DateOffType = {
          Sunday: item.Sunday,
          Monday: item.Monday,
          Tuesday: item.Tuesday,
          Wednesday: item.Wednesday,
          Thursday: item.Thursday,
          Friday: item.Friday,
          Saturday: item.Saturday,
        };
        setWeekAva(availability);
      } catch (e) {
        console.error("Failed to fetch doctor availability:", e);
      }
    };

    fetchAvailability();
  }, [SelectedDoctor]);

  const ListDoctors = ListDocs?.dataDoc.map((e) => ({
    name: e.name,
    uid: e.uid,
    Hospital: e.Hospital,
  }));

  useEffect(() => {
    if (
      weekAva &&
      SelectedDoctor &&
      SelectedDate &&
      !isAvailable(new Date(Date.parse(SelectedDate)))
    ) {
      alert("Sorry, this doctor is not available on that date");
      setSelectedDate(""); // Reset invalid date
    }
  }, [weekAva]);

  const dayMap = [
    "Sunday",
    "Monday",
    "Tuesday",
    "Wednesday",
    "Thursday",
    "Friday",
    "Saturday",
  ];
  useEffect(() => {
    const fetchTimeSlots = async () => {
      if (!SelectedDoctor?.uid || !SelectedDate) return;

      try {
        const response = await fetch(`${baseURL}/getTimeSlots`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ uid: SelectedDoctor.uid, date: SelectedDate }),
        });

        const data = await response.json();

        if (data.available && Array.isArray(data.availableSlots)) {
          setAvaTimes(data.availableSlots);
        } else {
          setAvaTimes([]);
          alert(`Doctor is not available on ${data.day()}`);
        }
      } catch (err) {
        console.error("Error fetching time slots:", err);
        setAvaTimes([]);
      }
    };

    fetchTimeSlots();
  }, [SelectedDoctor, SelectedDate]);

  const isAvailable = (date: Date): boolean => {
    if (!weekAva) return false;

    const dayKey = dayMap[date.getDay()] as keyof DateOffType;

    return weekAva[dayKey];
  };

  return (
    <div style={{ backgroundColor: "white", height: "100vh", width: "100vw" }}>
      <div style={{ display: "flex" }}>
        <button
          style={{
            background: "none",
            border: "none",
            position: "absolute",
            display: "flex",
            cursor: "pointer",
            marginTop: "5vh",
            marginLeft: "3vw",
            fontSize: 30,
          }}
          onClick={() => navigate(-1)}
        >
          X
        </button>
      </div>

      <div
        style={{
          flexDirection: "column",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          marginTop: "15vh",
        }}
      >
        <select
          style={{
            fontSize: "20px",
            fontWeight: "bold",
            padding: "12px 16px",
            border: "none",
            borderRadius: "10px",
            width: "32vw",
            backgroundColor: "rgb(233, 233, 233)",
            height: "8vh",
            marginTop: "1vh",
          }}
          onChange={(e) => {
            const selected = ListDoctors?.find(
              (doc) => doc.uid === e.target.value
            );
            if (selected) {
              SetSelectedDoctor({
                name: selected.name!,
                uid: selected.uid!,
                Hospital: selected.Hospital!,
              });

              if (
                weekAva &&
                SelectedDate &&
                !isAvailable(new Date(Date.parse(SelectedDate)))
              ) {
                setSelectedDate("");
                alert("Sorry, this doctor is not available on that date");
              }
            }
          }}
          value={SelectedDoctor?.uid ?? ""}
        >
          <option value="" disabled>
            Select A Doctor
          </option>
          {ListDoctors?.map((e, index) => (
            <option key={index} value={e.uid}>
              {e.name}
            </option>
          ))}
        </select>

        <input
          disabled={!SelectedDoctor?.uid}
          aria-label="appointment-date"
          type="date"
          value={SelectedDate}
          min={new Date().toISOString().split("T")[0]}
          onChange={(e) => {
            const newDate = new Date(Date.parse(e.target.value));
            setSelectedDate(e.target.value);

            if (SelectedDoctor && weekAva && !isAvailable(newDate)) {
              alert(
                `Doctor is not available on ${
                  dayMap[newDate.getDay()]
                }, please select another day`
              );
            }
          }}
          style={{
            width: "30vw",
            height: "6vh",
            border: "none",
            backgroundColor: "rgb(233, 233, 233)",
            borderRadius: "20px",
            fontWeight: "bold",
            marginTop: "2vh",
            fontSize: "20px",
            padding: "12px 16px",
          }}
        />

        <div
          style={{
            width: "30vw",
            overflow: "auto",
            display: "flex",
            flexDirection: "row",
            height: "14vh",
          }}
        >
          {SelectedDoctor?.uid && SelectedDate
            ? avaTimes.map((e) => (
                <button
                  onClick={() => setSelectedTime(e)}
                  key={e}
                  style={{
                    height: "8vh",
                    marginTop: "2vh",
                    fontWeight: SelectedTime === e ? 800 : 500,
                    minWidth: "9vw",
                    backgroundColor:
                      SelectedTime === e
                        ? "rgba(0, 164, 201, 0.62)"
                        : "rgb(233, 233, 233)",
                    color: SelectedTime === e ? "white" : "black",
                    borderRadius: "10px",
                    marginLeft: "1vw",
                    fontSize: SelectedTime === e ? "20px" : "15px",
                  }}
                >
                  {e}
                </button>
              ))
            : null}
        </div>

        <h2>📍 {SelectedDoctor?.Hospital}</h2>
        <textarea
          disabled={!SelectedDoctor?.uid}
          maxLength={200}
          onChange={(e) => setDesc(e.target.value)}
          style={{
            backgroundColor: "rgb(233, 233, 233)",
            borderRadius: "20px",
            width: "30vw",
            height: "12vh",
            border: "none",
            padding: "10px",
            fontSize: "15px",
          }}
        ></textarea>

        <button
          style={{
            backgroundColor: "rgba(4, 230, 0, 0.77)",
            height: "8vh",
            width: "20vw",
            borderRadius: "30px",
            color: "white",
            marginTop: "3vh",
            justifyContent: "center",
            fontSize: 20,
            fontWeight: "bold",
          }}
          onClick={() => {
            if (!SelectedDoctor || !SelectedDate || !SelectedTime) {
              alert("Please select all fields before booking.");
              return;
            }
            bookApt(
              SelectedDoctor,
              new Date(Date.parse(SelectedDate)),
              SelectedTime,
              desc,
              SelectedDoctor?.Hospital
            ).then((e) => {
              if (e == 200) {
                navigate("/home");
              }
            });
            // Booking API call can go here
          }}
        >
          Book
        </button>
      </div>
    </div>
  );
};

export default BookAppointment;
