...
const BookAppointment = () => {
  ...
  const ListDoctors = ListDocs?.dataDoc.map((e) => e.name);
  ...
  const selectedDoc = ListDocs?.dataDoc.find(
    (doc) => doc.name === SelectedDoctor
  );
  const dateOff = ListDocs?.dataWeek.find(
    (week) => week.uid === selectedDoc?.uid
  );
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (!isAvailable(new Date(Date.parse(SelectedDate)))) {
              setSelectedDate("");
              alert("Sorry this doctor is not available at that date");
            }
          }}
