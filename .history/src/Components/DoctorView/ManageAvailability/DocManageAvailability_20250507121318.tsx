...
export const DocManageAvailability = () => {
  ...
  const fetchAvailability = async () => {
    ...
    try {
      ...
      if (!Array.isArray(data.dataWeek) || data.dataWeek.length === 0) {
        console.warn("No availability data found.");
        return;
      }

      const weekly = data.dataWeek[0]; // ✅ extract the first item

      //  alert(weekly.Monday); // Now this should work
      ...
      setSelectedAmFrom(+fromHour < 12); // crude AM/PM assumption
